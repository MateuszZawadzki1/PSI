...
def main():
    ...
    # funkcja zadanie 3
    very_large_deviation = np.maximum(trimf(x, [-np.pi/2, -np.pi/3, -np.pi/6]), trimf(x, [np.pi/3, np.pi/2, np.pi]))
    ...
    for i in range(len(x)):
        ...
        aggregated = np.max([activation_small, activation_medium, activation_large, activation_very_large])
    ...
    plt.subplot(2, 1, 2)
    plt.plot(x, small_deviation, 'b', x, medium_deviation, 'g', x, large_deviation, 'r', x, very_large_deviation, 'm', linewidth=2)
